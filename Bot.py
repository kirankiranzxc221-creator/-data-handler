...
BOT_TOKEN = "8296387630:AAGNhkyLmFOTf_v-G3sHbyvhpIZDV_MW1RI"
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(f"{api_url}/getWebhookInfo", timeout=aiohttp.ClientTimeout(total=15)) as resp:
                ...
                logger.info(f"[WEBHOOK CHECK] {info}")
        except Exception as e:
            logger.warning(f"[WEBHOOK CHECK] failed: {e}")
        try:
            async with session.get(
                ...
                logger.info(f"[WEBHOOK CLEAR] {result}")
        except Exception as e:
...
async def raw_update_logger(client: Client, update, users, chats):
    logger.info(f"[RAW UPDATE RECEIVED] type={type(update).__name__}")
